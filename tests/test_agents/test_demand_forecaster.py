...
from src.agents.demand_forecaster import DemandForecaster
...
def test_triggers_contains_expected_event_types():
    assert DemandForecaster.TRIGGERS == ["demand_spike", "seasonal_change"]


def test_agent_discovered_by_plugin_registry():
    session = seeded_session()
    agents = PluginRegistry(session).get_agents_for_event("demand_spike")
    assert any(agent.name == "DemandForecaster" for agent in agents)


def test_returns_valid_non_negative_scenarios():
    session = seeded_session()
    event_row = session.query(Event).filter_by(event_type="demand_spike").first()
    scenarios = DemandForecaster(session).evaluate({"event": event_row, "config": load_config()})
