...
from src.agents.inventory_optimizer import InventoryOptimizer
...
def test_triggers_contains_expected_event_types():
    assert InventoryOptimizer.TRIGGERS == ["inventory_alert", "demand_spike"]


def test_agent_discovered_by_plugin_registry():
    session = seeded_session()
    agents = PluginRegistry(session).get_agents_for_event("inventory_alert")
    assert any(agent.name == "InventoryOptimizer" for agent in agents)


def test_returns_valid_scenarios():
    session = seeded_session()
    event_row = session.query(Event).filter_by(event_type="inventory_alert").first()
    scenarios = InventoryOptimizer(session).evaluate({"event": event_row, "config": load_config()})
