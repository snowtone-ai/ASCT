...
from src.agents.supply_risk_assessor import SupplyRiskAssessor
...
def test_triggers_contains_expected_event_types():
    assert SupplyRiskAssessor.TRIGGERS == ["supply_disruption", "weather_event"]


def test_agent_discovered_by_plugin_registry():
    session = seeded_session()
    agents = PluginRegistry(session).get_agents_for_event("supply_disruption")
    assert any(agent.name == "SupplyRiskAssessor" for agent in agents)


def test_returns_valid_scenarios():
    session = seeded_session()
    event_row = session.query(Event).filter_by(event_type="supply_disruption").first()
    scenarios = SupplyRiskAssessor(session).evaluate({"event": event_row, "config": load_config()})
